...
async def check_account_admin_rights(
    ...
    try:
        ...
        client = await telegram_service.get_client(telegram_session)
        ...
        try:
            ...
            logger.error(f"Ошибка проверки админ прав для {session_id} в {group_id}: {str(e)}")
            return {
                "is_admin": False,
                "permissions": [],
        ...
        logger.error(f"Ошибка подключения к Telegram для аккаунта {session_id}: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
