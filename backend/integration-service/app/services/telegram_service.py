        ...
        try:
            ...
            if auth_request.code:
                ...
                try:
                    ...
                    # ✅ НОВАЯ ЛОГИКА: Если пароль 2FA передан вместе с кодом, сразу пробуем его
                    if auth_request.password:
                        logger.info(f"🔐 SessionPasswordNeededError получен, но пароль 2FA передан - пробуем сразу")
                        try:
                            # Используем тот же клиент для ввода пароля
                            ...
                            logger.info(f"✅ 2FA password authentication successful for {auth_request.phone}")
                            ...
                            # Неверный пароль - сохраняем клиент для повтора
                            logger.warning(f"❌ Invalid 2FA password for {auth_request.phone}")
                            ...
                            error_msg = str(e)
                            logger.error(f"❌ Error during immediate 2FA: {e}")
                            # При ошибке - отключаем и очищаем
                            await client.disconnect()
                            await self._delete_auth_session(auth_key)
                            return TelegramConnectResponse(
                                status="error",
                        ...
                        # Пароль не передан - сохраняем сессию и просим пароль
                        # Обновляем auth session с информацией о 2FA
                        _GLOBAL_AUTH_SESSIONS[auth_key]['requires_2fa'] = True
            ...
            if auth_request.password:
                ...
                try:
                    ...
                    if "SESSION_PASSWORD_NEEDED" in error_msg or "session password needed" in error_msg.lower():
                        ...
                        if auth_request.password:
                            logger.info(f"🔐 RpcError SESSION_PASSWORD_NEEDED получен, но пароль 2FA передан - пробуем сразу")
                            try:
                                # Используем тот же клиент для ввода пароля
                                await client.sign_in(password=auth_request.password)
                                ...
                                logger.info(f"✅ 2FA password authentication successful (RpcError) for {auth_request.phone}")
                                ...
                                # Неверный пароль - сохраняем клиент для повтора
                                logger.warning(f"❌ Invalid 2FA password (RpcError) for {auth_request.phone}")
                                ...
                                logger.error(f"❌ Error during immediate 2FA (RpcError): {e2}")
                                # При ошибке - отключаем и очищаем
                                await client.disconnect()
                                await self._delete_auth_session(auth_key)
                                return TelegramConnectResponse(
                                    status="error",
                                    message=f"Ошибка 2FA входа: {str(e2)}"
                                )
                        else:
                            # Пароль не передан - сохраняем сессию и просим пароль
                            _GLOBAL_AUTH_SESSIONS[auth_key]['requires_2fa'] = True
        ...
        try:
            ...
            try:
                ...
                if "Two-steps verification is enabled and a password is required" in error_msg:
                    ...
                    logger.info(f"🔐 DEBUG: About to return 2fa_required status for user {user_id}")
                    ...
                    logger.info(f"🔐 DEBUG: Returning response: {response.status} - {response.message}")
                    return response
            
            logger.info(f"🔍 DEBUG: Continuing after QR recreate check for user {user_id}")
            
            # ✅ НОВАЯ ЛОГИКА: Проверяем авторизацию с обработкой 2FA
            try:
                logger.info(f"🔍 DEBUG: Checking client authorization for user {user_id}")
                ...
                logger.info(f"🔐 DEBUG: Caught SessionPasswordNeededError, about to return 2fa_required")
                ...
                logger.info(f"🔐 DEBUG: SessionPasswordNeededError response: {response.status} - {response.message}")
                return response
            except Exception as e:
                error_msg = str(e)
                logger.error(f"❌ Error checking QR authorization: {e}")
                logger.info(f"🔍 DEBUG: Exception in client.is_user_authorized() check: {error_msg}")
                ...
                if "SESSION_PASSWORD_NEEDED" in error_msg or "session password needed" in error_msg.lower():
                    ...
                    logger.info(f"🔐 DEBUG: Detected SESSION_PASSWORD_NEEDED in exception, returning 2fa_required")
