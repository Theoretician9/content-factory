#!/usr/bin/env python3
"""
Скрипт для замены файла telegram.py на исправленную версию с изоляцией пользователей.
"""

import os
import shutil

TELEGRAM_ENDPOINTS_PATH = "app/api/v1/endpoints/telegram.py"

FIXED_CODE = '''from fastapi import APIRouter, Depends, HTTPException, status, Query, Request
...
def main():
    # Создаем бэкап
    if os.path.exists(TELEGRAM_ENDPOINTS_PATH):
        backup_path = f"{TELEGRAM_ENDPOINTS_PATH}.backup"
        shutil.copy2(TELEGRAM_ENDPOINTS_PATH, backup_path)
        print(f"Создан бэкап: {backup_path}")
    
    # Записываем исправленный код
    with open(TELEGRAM_ENDPOINTS_PATH, 'w', encoding='utf-8') as f:
        f.write(FIXED_CODE)
    
    print(f"Файл {TELEGRAM_ENDPOINTS_PATH} обновлен с изоляцией пользователей!")
