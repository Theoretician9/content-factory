...
Внутри контейнера доступна команда `vault`. Обращаться нужно к серверу Vault по имени сервиса в сети Docker (порт 8201):
...
(Использование `127.0.0.1:8200` внутри контейнера может дать «connection refused» — в этом случае используй `http://vault:8201`.)
