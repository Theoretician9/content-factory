...
Внутри контейнера доступна команда `vault`. Для выполнения команд под админом Vault нужен root-токен (тот же, что используется для unseal или настройки). Задай его в этой оболочке:
...
(Если в контейнере Vault слушает на другом порту, подставь его. Узнать можно из логов или конфига Vault.)

Альтернатива: если на своей машине установлен `vault` CLI и поднят SSH-туннель на порт 8201 (см. Rules.md), можно выполнять те же команды локально, задав `VAULT_ADDR=http://127.0.0.1:8201` и `VAULT_TOKEN=<root_token>`.
