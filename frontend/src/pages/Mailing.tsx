...
// Интерфейс для ответа проверки админских прав
...
const Mailing = () => {
  ...
  useEffect(() => {
    loadTasks();
    loadAccounts();
    loadParseTasks(); // Загружаем сразу для секции выбора базы данных
  ...
  const handleCreateTask = async () => {
    ...
    if (!adminCheckResult || !adminCheckResult.can_proceed) {
      setCreateError('Сначала нужно указать группу и проверить администраторские права');
      return;
    }

    if (!selectedDataSource) {
      setDataSourceError('Выберите источник данных для аудитории');
      return;
    }
    ...
    try {
      // Автогенерация названия из группы
              ...
              <div className="p-6">
                ...
                <div className="max-w-4xl">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    {/* Выбор базы данных - ОБЯЗАТЕЛЬНО */}
                    <div className="md:col-span-2">
                      <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">
                        📂 Выбор базы данных для аудитории *
                      </h3>
                      
                      {dataSourceError && (
                        <div className="mb-4 p-3 bg-red-100 border border-red-400 text-red-700 rounded-lg">
                          <span className="mr-2">❌</span>{dataSourceError}
                        </div>
                      )}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <label className="cursor-pointer">
                          ...
                          <div className={`p-4 border-2 rounded-lg transition-colors ${
                            ...
                            <p className="text-sm text-gray-600 dark:text-gray-400">
                              Использовать ранее собранную базу аудитории из системы парсинга
                            </p>
                          </div>
                        </label>
                        ...
                        <label className="cursor-pointer">
                          ...
                          <div className={`p-4 border-2 rounded-lg transition-colors ${
                            ...
                            <p className="text-sm text-gray-600 dark:text-gray-400">
                              Загрузить файл с контактами в формате CSV, TXT или JSON
                            </p>
                          </div>
                        </label>
                      </div>
                      ...
                      {selectedDataSource === 'file' && (
                        <div className="mt-4 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                          ...
                          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                            Поддерживаемые форматы: CSV, TXT, JSON
                          </p>
                        </div>
                      )}
                    </div>
                    
                    {/* Только основные настройки безопасности */}
                    <div className="md:col-span-2">
                      <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">
                      ...
                      <div className="bg-blue-50 dark:bg-blue-900 p-4 rounded-lg">
                        <div className="flex items-center mb-2">
                          ...
                          <h4 className="font-medium text-blue-900 dark:text-blue-100">
                            Автоматические безопасные настройки
                          </h4>
                        </div>
                        <ul className="text-sm text-blue-800 dark:text-blue-200 space-y-1">
                          <li>• Задержка между приглашениями: 45 секунд (рекомендовано ТЗ)</li>
                          <li>• Размер батча: 5 пользователей в час (безопасный лимит)</li>
                          <li>• Автоматическое соблюдение лимитов Telegram</li>
                          <li>• Защита от FloodWait и блокировок</li>
                        </ul>
                      </div>
                    </div>
                  </div>
